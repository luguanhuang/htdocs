...
class Channel extends AtherFrameWork{
	...
	public function getChdata($param)
	{
		...
		$retTab =  $merchantsResult_DB->getChannelTabData($arrDev[0], " and chid=$arrDev[1]");
		if ($retTab["result"] != 0)
	    {
	...
	public function getChannelInfo($devname){
		...
		$retTab =  $merchantsResult_DB->getChannelTabData($devname,"");
		if ($retTab["result"] != 0)
	    {
	...
	{
		$var=explode(",",$param);
		TLOG_MSG("Station::AddWriteData: func begin");
		$merchantsResult_DB = new IMerchantsResult_DB($this->shandanDBFlg);
