...
class IMerchantsResult_DB 
{
    ...
    function getCompanyDetail($page)
   {
       ...
       $totalSql = "select id, companyname, status,created_date from dtm_company where is_deleted='0' ";
    ...
    function getAlarmConf($dev)
   {
       TLOG_MSG("IMerchantsResult_DB::getAlarmConf: func begin");
       $htmlRes = array();
       $returnArray = array("result"=>0,
           "data_list"=>array(),
           "totalNum"=>0,
       ...
       $alarm = $dev."_alaconf";
       $totalSql = "select chid, tagid, tagname, alarmontext from $alarm where previousvalue=1";
       
       TLOG_MSG("IMerchantsResult_DB::getAlarmConf: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'SELECT');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::getAlarmConf: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
           $returnArray["result"] = $ret["result"];
           $returnArray["sErrMsg"] = $ret["errorMsg"];
           return $returnArray;
       }
       ...
       $returnArray["data_list"] = $ret["data_list"];
       TLOG_MSG("IMerchantsResult_DB::getAlarmConf: func end");
       return $returnArray;
   }
   
   function getAlarmData($page)
   {
   ...
   {
       TLOG_MSG("IMerchantsResult_DB::getDeviceInfo: func begin");
       $htmlRes = array();
       $returnArray = array("result"=>0,
           "data_list"=>array(),
           "totalNum"=>0,
       ...
       $totalSql = "select id, devname,pic1filelocation,pic2filelocation,pic3filelocation,pic4filelocation,mainpagediv from auth where devname='$devname'";
       
       TLOG_MSG("IMerchantsResult_DB::getDeviceInfo: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'SELECT');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::getDeviceInfo: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
           $returnArray["result"] = $ret["result"];
           $returnArray["sErrMsg"] = $ret["errorMsg"];
           return $returnArray;
       }
       ...
       $returnArray["data_list"] = $ret["data_list"];
       TLOG_MSG("IMerchantsResult_DB::getDeviceInfo: func end");
       return $returnArray;
   }
   ...
   function getChannelidhourly($devname)
   {
       ...
       $totalSql = "select ch, dtype, slaveid,funcode,startreg,countreg from $devname where reporttype=2 ";
    ...
    function getChannelTabData($devname,$option)
   {
       ...
       $totalSql = "select id, chid, tagid,tagname,tagdesc,writeenable,coiltype,mainpagelocation from $tabname  $option";
   ...
   function getChannelTimeData($devname, $channelid, $tagid,$from,$to)
   {
       TLOG_MSG("IMerchantsResult_DB::getChannelTimeData: func begin");
       $htmlRes = array();
       $returnArray = array("result"=>0,
           "data_list"=>array(),
           "totalNum"=>0,
       ...
       $field = "data_".$tagid;
       $totalSql = "select id, time, $field from $tabname  where time>='$from' and time <='$to'";
       
       TLOG_MSG("IMerchantsResult_DB::getChannelTimeData: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'SELECT');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::getChannelTimeData: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
           $returnArray["result"] = $ret["result"];
           $returnArray["sErrMsg"] = $ret["errorMsg"];
           return $returnArray;
       }
       ...
       $returnArray["data_list"] = $ret["data_list"];
       //TLOG_MSG("IMerchantsResult_DB::getChannelTimeData: func end");
       return $returnArray;
   }
    ...
    function getChannelInfo($page, $devname)
   {
       ...
       $totalSql = "select ch, dtype, slaveid,funcode,startreg,countreg,active,company_id,group_id,station_id,history,chdesc,reporttype,Alarmchannel from $devname ";
    ...
    function getGroupDetail1($param)
   {
       ...
       $returnArray = array("result"=>0,
        ...
        if ($data[1] == "1")
        {
                $actualCntSql = "select count(*) as count from dtm_group where is_deleted='0' ";
                TLOG_MSG("IMerchantsResult_DB::getGroupDetail: [sql]=".$actualCntSql);
        }
       else if ($data[1] == "2" || $data[1] == "5")
        {
                $actualCntSql = "select count(*) as count from dtm_group where company_id=$data[2] and is_deleted='0' ";
                TLOG_MSG("IMerchantsResult_DB::getGroupDetail: [sql]=".$actualCntSql);
        }
        else if ($data[1] == "3"  || $data[1] == "6")
        {
                $actualCntSql = "select count(*) as count from dtm_group where company_id=$data[2] and id=$data[3] and is_deleted='0'  ";
                TLOG_MSG("IMerchantsResult_DB::getGroupDetail: [sql]=".$actualCntSql);
        }
        else if ($data[1] == "4"  || $data[1] == "7")
        {
                $actualCntSql = "select count(*) as count from dtm_group where company_id=$data[2] and id=$data[3] and is_deleted='0'  ";
                TLOG_MSG("IMerchantsResult_DB::getGroupDetail: [sql]=".$actualCntSql);
        }
       ...
       if ($data[1] == "1")
        {
                $totalSql = "select id, name, company_id,status from dtm_group where is_deleted='0'  ";
                TLOG_MSG("IMerchantsResult_DB::getGroupDetail: [sql]=".$actualCntSql);
        }
       else if ($data[1] == "2" || $data[1] == "5")
        {
                $totalSql = "select id, name, company_id,status from dtm_group  where company_id=$data[2] and is_deleted='0'  ";
                TLOG_MSG("IMerchantsResult_DB::getGroupDetail: [sql]=".$actualCntSql);
        }
        else if ($data[1] == "3" || $data[1] == "6")
        {
                $totalSql = "select id, name, company_id,status from dtm_group  where company_id=$data[2]  and id=$data[3] and is_deleted='0'  ";
                TLOG_MSG("IMerchantsResult_DB::getGroupDetail: [sql]=".$actualCntSql);
        }
       else if ($data[1] == "4" || $data[1] == "7")
        {
    ...
    function getStationList($param)
   {
       ...
       $returnArray = array("result"=>0,
        ...
        if ($data[1] == "1")
        {
                $actualCntSql = "select count(*) as count from dtm_station where is_deleted='0' ";
                TLOG_MSG("IMerchantsResult_DB::getStationList: [sql]=".$actualCntSql);
        }
       else if ($data[1] == "2" || $data[1] == "5")
        {
                $actualCntSql = "select count(*) as count from dtm_station where company_id=$data[2] and is_deleted='0' ";
                TLOG_MSG("IMerchantsResult_DB::getStationList: [sql]=".$actualCntSql);
        }
        else if ($data[1] == "3" || $data[1] == "6")
        {
                $actualCntSql = "select count(*) as count from dtm_station where company_id=$data[2] and group_id=$data[3] and is_deleted='0' ";
                TLOG_MSG("IMerchantsResult_DB::getStationList: [sql]=".$actualCntSql);
        }
        else if ($data[1] == "4" || $data[1] == "7")
        {
                $actualCntSql = "select count(*) as count from dtm_station where company_id=$data[2] and group_id=$data[3] and id=$data[4] and is_deleted='0' ";
                TLOG_MSG("IMerchantsResult_DB::getStationList: [sql]=".$actualCntSql);
        }
       ...
       if ($data[1] == "1")
        {
                $totalSql = "select id, name, company_id, group_id, status from dtm_station where is_deleted='0' ";
                
                TLOG_MSG("IMerchantsResult_DB::getStationList: [sql]=".$actualCntSql);
        }
       else if ($data[1] == "2" || $data[1] == "5")
        {
                $totalSql = "select id, name, company_id, group_id, status from dtm_station  where company_id=$data[2] and is_deleted='0'  ";
            
                TLOG_MSG("IMerchantsResult_DB::getStationList: [sql]=".$actualCntSql);
        }
        else if ($data[1] == "3" || $data[1] == "6")
        {
                $totalSql = "select id, name, company_id, group_id, status from dtm_station where company_id=$data[2] and group_id=$data[3] and is_deleted='0' ";
                TLOG_MSG("IMerchantsResult_DB::getStationList: [sql]=".$actualCntSql);
        }
       else if ($data[1] == "4" || $data[1] == "7")
        {
                $totalSql = "select id, name, company_id, group_id, status from dtm_station where company_id=$data[2] and group_id=$data[3] and id=$data[4] and is_deleted='0' ";
                TLOG_MSG("IMerchantsResult_DB::getStationList: [sql]=".$actualCntSql);
        }
    ...
    function getDeviceList($param)
   {
       ...
       if ($data[1] == "1")
        {
                $totalSql = "select id, macid, devname,ptype,servip,servport,retry,timeout,polltime,company_id,group_id,station_id,active,socktype,devicedesc,templatelocation,connected,pic1filelocation,pic2filelocation,pic3filelocation,pic4filelocation,mainpagediv from auth ";
                TLOG_MSG("IMerchantsResult_DB::getDeviceList: [sql]=".$actualCntSql);
        }
       else if ($data[1] == "2" || $data[1] == "5")
        {
                $totalSql = "select id, macid, devname,ptype,servip,servport,retry,timeout,polltime,company_id,group_id,station_id,active,socktype,devicedesc,templatelocation,connected,pic1filelocation,pic2filelocation,pic3filelocation,pic4filelocation,mainpagediv from auth where company_id=$data[2] ";
                TLOG_MSG("IMerchantsResult_DB::getDeviceList: [sql]=".$actualCntSql);
        }
        else if ($data[1] == "3" || $data[1] == "6")
        {
                $totalSql = "select id, macid, devname,ptype,servip,servport,retry,timeout,polltime,company_id,group_id,station_id,active,socktype,devicedesc,templatelocation,connected,pic1filelocation,pic2filelocation,pic3filelocation,pic4filelocation,mainpagediv from auth  where company_id=$data[2] and group_id=$data[3] ";
                TLOG_MSG("IMerchantsResult_DB::getDeviceList: [sql]=".$actualCntSql);
        }
        else if ($data[1] == "4" || $data[1] == "7")
        {
                $totalSql = "select id, macid, devname,ptype,servip,servport,retry,timeout,polltime,company_id,group_id,station_id,active,socktype,devicedesc,templatelocation,connected,pic1filelocation,pic2filelocation,pic3filelocation,pic4filelocation,mainpagediv from auth where company_id=$data[2] and group_id=$data[3] and station_id= $data[4] ";
                TLOG_MSG("IMerchantsResult_DB::getDeviceList: [sql]=".$actualCntSql);
        }
   ...
   {
       TLOG_MSG("IMerchantsResult_DB::getConnList: func begin");
       $htmlRes = array();
       $returnArray = array("result"=>0,
           "data_list"=>array(),
           "totalNum"=>0,
        ...
        $totalSql = "select id, macid, devname,ptype,servip,servport,retry,timeout,polltime,company_id,group_id,station_id,active,socktype,devicedesc,templatelocation,connected from auth ";
       
       TLOG_MSG("IMerchantsResult_DB::getConnList: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'SELECT');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::getConnList: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
           $returnArray["result"] = $ret["result"];
           $returnArray["sErrMsg"] = $ret["errorMsg"];
           return $returnArray;
       }
       ...
       $returnArray["data_list"] = $ret["data_list"];
       TLOG_MSG("IMerchantsResult_DB::getConnList: func end");
       return $returnArray;
   }
    ...
    function getUserList($param)
   {
       ...
       TLOG_MSG("IMerchantsResult_DB::getUserList: param=".$param);
        if ($data[1] == "1")
        {
                $actualCntSql = "select count(*) as count from dtm_user where is_deleted='0'";
                TLOG_MSG("IMerchantsResult_DB::getUserList: [sql]=".$actualCntSql);
        }
       else if ($data[1] == "2" || $data[1] == "5")
        {
            $actualCntSql = "select count(*) as count from dtm_user where roll_id in(2,3,4,5,6,7) and company_id=$data[2] and is_deleted='0' ";
                TLOG_MSG("IMerchantsResult_DB::getUserList: [sql]=".$actualCntSql);
        }
        else if ($data[1] == "3" || $data[1] == "6")
        {
            $actualCntSql = "select count(*) as count from dtm_user where roll_id in(3,4,6,7) and company_id=$data[2] and group_id=$data[3] and is_deleted='0' ";
                TLOG_MSG("IMerchantsResult_DB::getUserList: [sql]=".$actualCntSql);
        }
        else if ($data[1] == "4" || $data[1] == "7")
        {
            $actualCntSql = "select count(*) as count from dtm_user where roll_id in(4,7) and company_id=$data[2] and group_id=$data[3] and station_id=$data[4] and is_deleted='0' ";
                TLOG_MSG("IMerchantsResult_DB::getUserList: [sql]=".$actualCntSql);
        }
       $returnArray["totalNum"] = $this->queryCount($actualCntSql);
       ...
       if ($data[1] == "1")
        {
            $totalSql = "select id, name, username,password,writepassword,email,suburb,country,address,postcode,phone,mobile,company_id,group_id,station_id,fax,roll_id,active from dtm_user  where is_deleted='0' ";
            $totalSql = $totalSql." limit $numFrom, $pageSize";
        }
        else if ($data[1] == "2" || $data[1] == "5")
        {
            $totalSql = "select id, name, username,password,writepassword,email,suburb,country,address,postcode,phone,mobile,company_id,group_id,station_id,fax,roll_id,active from dtm_user where roll_id in(2,3,4,5,6,7) and company_id=$data[2] and is_deleted='0' ";
            $totalSql = $totalSql." limit $numFrom, $pageSize";
        }
       else if ($data[1] == "3" || $data[1] == "6")
        {
            $totalSql = "select id, name, username,password,writepassword,email,suburb,country,address,postcode,phone,mobile,company_id,group_id,station_id,fax,roll_id,active from dtm_user where roll_id in(3,4,6,7) and company_id=$data[2] and group_id=$data[3] and is_deleted='0' ";
            $totalSql = $totalSql." limit $numFrom, $pageSize";
        }
         else if ($data[1] == "4" || $data[1] == "7")
        {
            $totalSql = "select id, name, username,password,writepassword,email,suburb,country,address,postcode,phone,mobile,company_id,group_id,station_id,fax,roll_id,active from dtm_user where roll_id in(4,7) and company_id=$data[2] and group_id=$data[3] and station_id=$data[4] and is_deleted='0' ";
            $totalSql = $totalSql." limit $numFrom, $pageSize";
        }
    ...
    function AddStationInfo($param)
   {
       ...
       if (empty($param['active']))
        $totalSql = "insert into dtm_station(name, company_id,group_id,status,is_deleted,created_date,updated_date) values('$param[stationname]',$param[companyname],$param[groupname], '0','0','$time','$time')";
        else
            $totalSql = "insert into dtm_station(name, company_id,group_id,status,is_deleted,created_date,updated_date) values('$param[stationname]',$param[companyname],$param[groupname], '1','0','$time','$time')";
	 ...
	 function DelStationInfo($param)
   {
       TLOG_MSG("IMerchantsResult_DB::DelStationInfo: func begin");
       ...
       $totalSql = "update dtm_station set is_deleted='1' where id=$param[id]";
        
       TLOG_MSG("IMerchantsResult_DB::DelStationInfo: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'INSERT');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::DelStationInfo: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
           
       }
       
       TLOG_MSG("IMerchantsResult_DB::DelStationInfo: func end");
       return $ret;
   }
    ...
    function AddDeviceInfo($param)
   {
       ...
       $time=date("Y-m-d H-i-s", time());//默认就是time()，可以省略
       if (empty($param['active']))
        $totalSql = "insert into auth(macid, devname,ptype,servip,servport,retry,timeout,polltime,company_id,group_id,station_id,connected,active,socktype,threaded,devicedesc,templatelocation,pic1filelocation,pic2filelocation,pic3filelocation,pic4filelocation,mainpagediv) values('$param[macid]','$param[devicename]','$param[ptype]','$param[servip]','$param[servport]','$param[retry]','$param[timeout]','$param[polltime]','$param[companyname]','$param[groupname]','$param[stationname]', '0','0','1','0','$param[devicedesc]','$param[templatelocation]','$param[pic1filelocation]','$param[pic2filelocation]','$param[pic3filelocation]','$param[pic4filelocation]','$param[mainpagediv]')";
        else
            $totalSql = "insert into auth(macid, devname,ptype,servip,servport,retry,timeout,polltime,company_id,group_id,station_id,connected,active,socktype,threaded,devicedesc,templatelocation,pic1filelocation,pic2filelocation,pic3filelocation,pic4filelocation,mainpagediv) values('$param[macid]','$param[devicename]','$param[ptype]','$param[servip]','$param[servport]','$param[retry]','$param[timeout]','$param[polltime]','$param[companyname]','$param[groupname]','$param[stationname]', '0','1','$param[socktype]','0','$param[devicedesc]','$param[templatelocation]','$param[pic1filelocation]','$param[pic2filelocation]','$param[pic3filelocation]','$param[pic4filelocation]','$param[mainpagediv]')";
 ...
 `ch` int(0) NOT NULL AUTO_INCREMENT,
  ...
  `writelogin` varchar(50) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL,
  `writedatetime` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  `writeackdatetime` DATETIME NULL ,
 ...
 (
  ...
  `mainpageenable` enum('0','1') CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL DEFAULT NULL,
  `mainpagelocation` varchar(16) NULL DEFAULT NULL,
  PRIMARY KEY (`id`) USING BTREE
  ...
  `chid` int(0) NOT NULL,
  `tagid` int(0) NOT NULL,
  `tagname` varchar(128) NULL DEFAULT NULL,
  `alarmenable` enum('0','1') NULL DEFAULT NULL,
  `alarmontext` varchar(128) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL,
  `alarmofftext` text CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL,
  `minvalue` double NULL DEFAULT NULL,
  `maxval` double NULL DEFAULT NULL,
  `previousvalue` int(0) NULL DEFAULT '0',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci ROW_FORMAT = Dynamic;";
       TLOG_MSG("IMerchantsResult_DB::AddDeviceInfo: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'INSERT');
    ...
    $totalSql = "CREATE TABLE IF NOT EXISTS `$param[devicename]_alarm`  (
  `id` int(0) NOT NULL AUTO_INCREMENT,
  `alarm_name` varchar(128) NULL DEFAULT NULL,
  `alarmval` varchar(32) NULL DEFAULT NULL,
  `ack` enum('0','1') NULL DEFAULT NULL,
  `alarmdatetime` datetime DEFAULT CURRENT_TIMESTAMP,
  `ackdatetime` datetime DEFAULT NULL,
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci ROW_FORMAT = Dynamic;";
       TLOG_MSG("IMerchantsResult_DB::AddDeviceInfo: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'INSERT');
    ...
    function AddChannel($param)
   {
       ...
       if (!empty($param['Alarmchannel']))
        $Alarmchannel='1';
        $totalSql = "insert into $param[devname](dtype, slaveid,funcode,startreg,countreg,querystr,resstr,active,company_id,group_id,station_id,history,chdesc,reporttype, Alarmchannel) values('$param[dtype]','$param[slaveid]','$param[funcode]','$param[startreg]','$param[countreg]','$param[querystr]','$param[resstr]','$active',$param[companyname],$param[groupname],$param[stationname],'$history','$param[chdesc]','$param[reporttype]','$Alarmchannel')";
  ...
  PRIMARY KEY (`id`) USING BTREE
       ...
       for($i=1; $i<=$param[countreg];$i++)
       {
          ...
          for($i=1; $i<=$param[countreg];$i++)
       {
            $totalSql = "insert into $param[devname]_alaconf(chid, tagid) values($chid,$i)";
            TLOG_MSG("IMerchantsResult_DB::AddChannel: [sql]=".$totalSql);
           $ret = $this->DB->execute_v2($totalSql, 'INSERT');
           if ($ret["result"] != 0)
           {
    ...
    function EditChannel($param)
   {
       ...
       if ($param['history']!=null)
        $history='1';
         if ($param['Alarmchannel']!=null)
        $Alarmchannel='1';
       
        $totalSql = "update $param[devicename] set dtype='$param[dtype]', slaveid='$param[slaveid]',funcode='$param[funcode]',startreg='$param[startreg]',countreg='$param[countreg]',company_id=$param[companyname],group_id=$param[groupname],station_id=$param[stationname], active='$active', history='$history', chdesc='$param[chdesc]' , reporttype='$param[reporttype]' , Alarmchannel='$Alarmchannel' where ch=".$param['id'];
    ...
    function AddWriteData($device,$channel,$dtype,$slaveid,$funcode,$reg,$value)
   {
    ...
    TLOG_MSG("IMerchantsResult_DB::AddWriteData: [sql]=".$totalSql);
	 ...
	 function UpdateChInfo($device,$channel,$user)
   {
       TLOG_MSG("IMerchantsResult_DB::UpdateChInfo: func begin");
       $returnArray = array("result"=>0,
           "data_list"=>array(),
           "sErrMsg"=>"");
       $totalSql = "";
       $time=date("Y-m-d H-i-s", time());//默认就是time()，可以省略
       $name = $device."_write";
       $totalSql = "update $device set writelogin='$user',writedatetime='$time' where ch=".$channel;

    TLOG_MSG("IMerchantsResult_DB::UpdateChInfo: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'update');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::UpdateChInfo: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
           
       }
       
       TLOG_MSG("IMerchantsResult_DB::UpdateChInfo: func end");
       return $ret;
   }
    ...
    function EditTag($param)
   {
       ...
       for ($i=1; $i<=$param[countreg];$i++)
        {
            ...
            $mainpageenable = "mainpageenable_$i";
            $mainpagelocation = "mainpagelocation_$i";
            $excellocationinfo = $param[$excellocation];
            $mainpagelocationinfo = $param[$mainpagelocation];
           ...
           if (!empty($param[$writeenable]))
             ...
             if (!empty($param[$mainpageenable]))
               $mainpageenableflg='1';
            
            $totalSql = "update $param[devname]_tag set tagname='$param[$tagname]',tagdesc='$param[$tagdesc]',webshow='$webshowflg',coiltype='$coiltypeflg',writeenable='$writeenableflg',reportenable='$reportenableflg',excellocation='$excellocationinfo' ,mainpageenable='$mainpageenableflg',mainpagelocation='$mainpagelocationinfo' where chid=$param[$channelid] and tagid=$param[$tagid]";
    ...
    function editAlarm($param)
   {
       TLOG_MSG("IMerchantsResult_DB::editAlarm:  active=");
       $returnArray = array("result"=>0,
           "data_list"=>array(),
           "totalNum"=>0,
           "sErrMsg"=>"");
       ...
       for ($i=1; $i<=$param[countreg];$i++)
        {
            ...
            $alarmenable = "alarmenable_$i";
            $alarmontext = "alarmontext_$i";
            $alarmofftext = "alarmofftext_$i";
            $minvalue = "minvalue_$i";
            $maxval = "maxval_$i";
           ...
           if (!empty($param[$alarmenable]))
               $alarmenableflg='1';
           
           
            if (empty($param[$maxval]))
               $tmpmax=0;
			else
				$tmpmax = $param[$maxval];

            if (empty($param[$minvalue]))
               $tmpmin=0;
			else
				$tmpmin = $param[$minvalue];
            $totalSql = "update $param[devname]_alaconf set tagname='$param[$tagname]',alarmenable='$alarmenableflg',alarmontext='$param[$alarmontext]',alarmofftext='$param[$alarmofftext]',minvalue=$tmpmin,maxval=$tmpmax  where chid=$param[$channelid] and tagid=$param[$tagid]";
            
            TLOG_MSG("IMerchantsResult_DB::editAlarm: [sql]=".$totalSql);
           $ret = $this->DB->execute_v2($totalSql, 'update');
           if ($ret["result"] != 0)
           {
               TLOG_ERR("IMerchantsResult_DB::editAlarm: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
    ...
    function getTagInfo($devname,$id)
   {
       ...
       $returnArray = array("result"=>0,
           "data_list"=>array(),
           "sErrMsg"=>"");
       $totalSql = "select chid, tagid, tagname,tagdesc,webshow,coiltype,writeenable,reportenable,excellocation,mainpageenable,mainpagelocation from $name where chid=$id";
    ...
    function getAlarmInfo($devname,$id)
   {
       TLOG_MSG("IMerchantsResult_DB::getAlarmInfo: func begin");
       $name = $devname."_alaconf";
       $returnArray = array("result"=>0,
           "data_list"=>array(),
           "sErrMsg"=>"");
       $totalSql = "select chid, tagid, tagname, alarmenable,alarmontext,alarmofftext,minvalue,maxval from $name where chid=$id";
        
       TLOG_MSG("IMerchantsResult_DB::getAlarmInfo: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'SELECT');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::getAlarmInfo: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
           $returnArray["result"] = $ret["result"];
           $returnArray["sErrMsg"] = $ret["errorMsg"];
           return $returnArray;
       }
       ...
       $returnArray["data_list"] = $ret["data_list"];
       TLOG_MSG("IMerchantsResult_DB::getAlarmInfo: func end");
       return $returnArray;
   }
    ...
    function editDeviceInfo($param)
   {
       ...
       //$totalSql = "update auth set macid='$param[macid]'";//, devname='$param['devicename']',ptype='$param['ptype']',servip='$param['servip']',servport='$param['servport']',retry='$param['retry']',timeout='$param['timeout']',polltime='$param['polltime']',company_id='$param['companyname']',group_id='$param['groupname']',station_id='$param['stationname']',connected='$param['connected']', active='0',socktype='.$param['socktype']." where id=$param['id']";
        if ($param['active']==null)
            $totalSql = "update auth set macid='$param[macid]', devname='$param[devicename]',ptype='$param[ptype]',servip='$param[servip]',servport='$param[servport]',retry='$param[retry]',timeout='$param[timeout]',polltime='$param[polltime]',company_id='$param[companyname]',group_id='$param[groupname]',station_id='$param[stationname]',connected='0', active='0',socktype='$param[socktype]',devicedesc='$param[devicedesc]',templatelocation='$param[templatelocation]',pic1filelocation='$param[pic1filelocation]',pic2filelocation='$param[pic2filelocation]',pic3filelocation='$param[pic3filelocation]',pic4filelocation='$param[pic4filelocation]',mainpagediv='$param[mainpagediv]' where id=".$param['id'];
        else
            $totalSql = "update auth set macid='$param[macid]', devname='$param[devicename]',ptype='$param[ptype]',servip='$param[servip]',servport='$param[servport]',retry='$param[retry]',timeout='$param[timeout]',polltime='$param[polltime]',company_id='$param[companyname]',group_id='$param[groupname]',station_id='$param[stationname]',connected='0', active='1',socktype='$param[socktype]',devicedesc='$param[devicedesc]',templatelocation='$param[templatelocation]',pic1filelocation='$param[pic1filelocation]',pic2filelocation='$param[pic2filelocation]',pic3filelocation='$param[pic3filelocation]',pic4filelocation='$param[pic4filelocation]',mainpagediv='$param[mainpagediv]' where id=".$param['id'];
    ...
    function DelCompany($param)
   {
       ...
       $returnArray = array("result"=>0,
        ...
        $totalSql = "update dtm_company  set is_deleted='1' where id=$param[id]";
       TLOG_MSG("IMerchantsResult_DB::DelCompany: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'DELETE');
       if ($ret["result"] != 0)
       {
   ...
   {
       TLOG_MSG("IMerchantsResult_DB::DelUser: func begin");
       $returnArray = array("result"=>0,
           "sErrMsg"=>"");
       
        $totalSql = "update dtm_user set is_deleted='1' where id=$param[id]";
       TLOG_MSG("IMerchantsResult_DB::DelUser: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'UPDATE');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::DelUser: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
       }
       
       TLOG_MSG("IMerchantsResult_DB::DelUser: func end");
       return $ret;
   }
   
    function DelDev($param)
   {
       TLOG_MSG("IMerchantsResult_DB::DelDev: func begin");
       $returnArray = array("result"=>0,
           "sErrMsg"=>"");

       $totalSql = "delete from auth where devname='$param[devname]'";
       TLOG_MSG("IMerchantsResult_DB::DelTab: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'DELETE');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::DelTab: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
       }
		   
       $name = $param[devname]."_"."tag";
	     $totalSql = "drop table $name";
       TLOG_MSG("IMerchantsResult_DB::DelDev: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'INSERT');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::DelDev: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
       }
	   
	   $name = $param[devname]."_"."alarm";
	     $totalSql = "drop table $name";
       TLOG_MSG("IMerchantsResult_DB::DelDev: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'INSERT');
       if ($ret["result"] != 0)
       ...
       TLOG_MSG("IMerchantsResult_DB::DelDev: func end");
       return $ret;
   }
   
   
    function DelTab($param)
   {
       TLOG_MSG("IMerchantsResult_DB::DelTab: func begin");
       $returnArray = array("result"=>0,
           "sErrMsg"=>"");
       $name = $param[devname]."_"."tag";
        $totalSql = "delete from $name where id=$param[id]";
       TLOG_MSG("IMerchantsResult_DB::DelTab: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'DELETE');
       if ($ret["result"] != 0)
       {
       ...
       TLOG_MSG("IMerchantsResult_DB::DelTab: func end");
       return $ret;
   }
   
     function DelAlconfig($param)
   {
       TLOG_MSG("IMerchantsResult_DB::DelAlconfig: func begin");
       $returnArray = array("result"=>0,
           "sErrMsg"=>"");
       $name = $param[devname]."_"."alaconf";
        $totalSql = "delete from $name where tagid=$param[id]";
       TLOG_MSG("IMerchantsResult_DB::DelAlconfig: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'DELETE');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::DelAlconfig: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
       }
       
       TLOG_MSG("IMerchantsResult_DB::DelAlconfig: func end");
       return $ret;
   }
   
    function DelConnel($param)
   {
       TLOG_MSG("IMerchantsResult_DB::DelConnel: func begin");
       $returnArray = array("result"=>0,
           "sErrMsg"=>"");
       $name = $param[devname];
        $totalSql = "delete from $name where ch=$param[id]";
       TLOG_MSG("IMerchantsResult_DB::DelConnel: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'DELETE');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::DelConnel: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
       }
       
	   $name = $param[devname]."_".$param[id];
	     $totalSql = "drop table $name";
       TLOG_MSG("IMerchantsResult_DB::DelConnel: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'INSERT');
       if ($ret["result"] != 0)
       {
	    ...
	    $name = $param[devname]."_livedata_".$param[id];
	     $totalSql = "drop table $name";
       TLOG_MSG("IMerchantsResult_DB::DelConnel: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'INSERT');
       if ($ret["result"] != 0)
    ...
    function AddCompany($param)
   {
       ...
       $totalSql = "";
       if (empty($param['active']))
            $totalSql = "insert into dtm_company(companyname, status,is_deleted,created_date,updated_date) values('$param[companyname]','0','0','{$time}','{$time}')";
        else
            $totalSql = "insert into dtm_company(companyname, status,is_deleted,created_date,updated_date) values('$param[companyname]','1','0','{$time}','{$time}')";
   ...
   function AddGroup($param)
   {
       ...
       $totalSql = "";
       if (empty($param['active']))
            $totalSql = "insert into dtm_group(name,company_id, status,is_deleted,created_date,updated_date) values('$param[groupname]','$param[companyname]','0','0','{$time}','{$time}')";
        else
            $totalSql = "insert into dtm_group(name,company_id, status,is_deleted,created_date,updated_date) values('$param[groupname]','$param[companyname]','1','0','{$time}','{$time}')";
   ...
   {
       TLOG_MSG("IMerchantsResult_DB::delGroup: func begin");
       $returnArray = array("result"=>0,
           "data_list"=>array(),
           "sErrMsg"=>"");
       $time=date("Y-m-d H-i-s", time());//默认就是time()，可以省略
       ...
       $totalSql = "update dtm_group set is_deleted='1' where id=".$param['id'];
        
       TLOG_MSG("IMerchantsResult_DB::delGroup: [sql]=".$totalSql);
       $ret = $this->DB->execute_v2($totalSql, 'update');
       if ($ret["result"] != 0)
       {
           TLOG_ERR("IMerchantsResult_DB::delGroup: Call DB->execute_v2 error [error msg]=".$ret["errorMsg"]);
       }
       
       TLOG_MSG("IMerchantsResult_DB::delGroup: func end");
       return $ret;
   }
