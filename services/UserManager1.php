...
　　　　　　　　　　　　　　　'sessionid' : 当前登录SessionID
	*/
	public function login($user,$password,$vcode=""){
		...
		TLOG_MSG("UserManager::login: rolename=$rolename errorinfo=$errorinfo loginstatus=$loginstatus");
		...
		//验证提交密码格式
		elseif (!$this->validate_password($password)){$result=false;}
		//验证登录
		...
		$result = array_merge($this->Ary_UserAll[$user],array("sessionid"=>$sessionid));
