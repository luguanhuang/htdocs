...
global $Obj_Frame;
global $Ary_Result;
TLOG_INIT(TLOG_LEVEL_M, 10, 1024000, "./logs", "User",0);
TLOG_MSG("user: func begin name33=".$tmp." user=".$user);

$Obj_Frame = new AtherFrameWork();

$Ary_Result= $Obj_Frame->load_page("User::getUserList",@FuncExt::getnumber('page'),false);
...
<div class="listwrap">
  ...
  <div class="op"><a href="useradd.php">add user</a><div class="clear"></div></div>
    <table border="0" align="center" cellpadding="0" cellspacing="0" width="100%" class="listtab">
      ...
      <tbody>
		...
		<td><?php echo($row['username']); ?><input name="username_<?=$k?>" id="username_<?=$k?>" type="hidden" value="<?=$row['username']?>" /></td>
		...
		<td><?php echo($row['roll']); ?><input name="roll_<?=$k?>" id="roll_<?=$k?>" type="hidden" value="<?=$row['roll_id']?>" /></td>
		<td><?php if ($row['active']=="1") echo('active');
					else echo('deactive'); ?><input name="status_<?=$k?>" id="status_<?=$k?>" type="hidden" value="<?=$row['active']?>" /></td>
					
		<input name="mobile_<?=$k?>" style="display:none" id="mobile_<?=$k?>" type="hidden" value="<?=$row['mobile']?>" />
		<input name="email_<?=$k?>" style="display:none" id="email_<?=$k?>" type="hidden" value="<?=$row['email']?>" />
					
          <td><a href="#" submitwin="_self"  onclick="return User_Seting(<?=$k?>)">modify</a> | <a href="#" onclick="return Company_Delete(<?=$k?>)">delete</a></td>
        </tr>
        <?php }
		}
		?>
...
<form id="frm2" name="frm2" action="userset.php" onsubmit="" submitwin="_self" method="get">
	...
	<input name="roll" 		id="roll" 		type="hidden" value="" />
	<input name="status" 		id="status" 		type="hidden" value="" />
	<input name="id" 		id="id" 		type="hidden" value="" />
	<input name="mobile" 		id="mobile" 		type="hidden" value="" />
	<input name="email" 		id="email" 		type="hidden" value="" />
	
    <input name="php_parameter" type="hidden" id="php_parameter" value="['name','username','companyname','groupname','stationname','roll','status','id','mobile','email']" />
    <input name="php_returnmode" type="hidden" id="php_returnmode" value="normal" />
</form>
