...
function Create_RowString($str,$id){
	$str= $_G.strval($str);
	$id	= $_G.strval($id);
	if ($str!="" && $id!=""){
...
function AuthSvr_Addnew(){
	...
	if (document.all){
		...
		for ($c=0;$c<$ary_cell.length;$c++){
			...
			//加入内容
			$obj_list.rows[$int_rows].cells[$c].innerHTML = Create_RowString($obj_tmpl.cells[$c].innerHTML,$int_rnd);
		}
	}
	else{
		$obj_new.innerHTML = Create_RowString($obj_new.innerHTML,$int_rnd);
		$obj_tmpl.parentNode.appendChild($obj_new);
	}
}

function AuthSvr_Delete($id){
	$id = $_G.strval($id);
	if ($id==""){ return false; }
...
function AuthSvr_Validate($form){
	if (!$form){return $form;}
	//验证表单
	var	$ary_element = new Array();
		$ary_element["timeout"]	= new Array(1,	null,	'超时时长',		">0");
		$ary_element["maxtime"]	= new Array(1,	null,	'最大连接次数',	">0");
...
function SecClient_Delete($id,$isnew){
	$id = $_G.strval($id);
	if ($id==""){ return false; }
	var $obj_row = $('row_'+$id);
	if (!$obj_row){ return false; }
...
function SecClient_Validate($form){
	...
	if ($ary_val.length<1){
		$_C.Alert("至少要有一条安全客户端记录",null);
		return false;
	}
	$form.proto.value	= $('Protocol').value
	$form.cipher.value	= $('Encrypt').value
	$form.remotes.value = $ary_val.join(", ");
	//提交数据
	return AjaxSubmit(
		$form,
		{
