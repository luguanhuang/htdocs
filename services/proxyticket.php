...
<div class="listwrap">
  ...
  <div class="op">
   <input type="checkbox" name="log" id="log" value=<?=$Ary_Result2['result']['dzzf_audit']?> <?php if ($Ary_Result2['result']['dzzf_audit']=='1'){ echo('checked=true');} ?> /><span>记录日志</span>
  <input type="checkbox" name="mobi" id="mobi" value=<?=$Ary_Result2['result']['stamped']?> <?php if ($Ary_Result2['result']['stamped']=='1'){ echo(' checked=true'); }?> /><span>验签</span>
  <input name="php_interface" type="hidden" id="php_interface" value="NetConfig::get_proxytickets2" />
    <input name="php_parameter" type="hidden" id="php_parameter" value="[['log','mobi']]" />
    <input name="php_returnmode" type="hidden" id="php_returnmode" value="normal" />
   <input type="submit" id="btnsave2" name="btnsave2" value="保存配置" class="btn" />
